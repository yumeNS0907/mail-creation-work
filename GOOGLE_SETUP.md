...
すでに同じWebアプリを公開済みの場合は、「デプロイを管理」から対象デプロイを編集し、バージョンを「新バージョン」にして再デプロイしてください。月別表示と個別削除には、この最新版の `Code.gs` が必要です。
...
- 個別削除は管理者ページのボタンから行い、確認ダイアログの後にスプレッドシートの該当行を削除します。
